...
prompt = '''
  ...
  # 3.输出：实体和关系抽取结果，采用json格式，包含nodes字段和edges字段，分别对应实体和关系抽取结果。nodes字段是一个字典，键为实体提及，值为一个字典，有一个键type、其值为实体类型。edges字段是一个列表，每一个元素是一个三元组（列表），分别对应头实体提及、关系类型和尾实体提及。
    ...
    输出为{"nodes": {"Moscow": {"type": "loc"}, "Bolshoi_Ballet": {"type": "org"}, "Leningrad": {"type": "loc"}, "Kirov_Ballet": {"type": "org"}, "Armenian": {"type": "other"}, "Yerevan": {"type": "loc"}, "U.S.": {"type": "loc"}, "June_Anderson": {"type": "peop"}, "Carol_Vaness": {"type": "peop"}, "British": {"type": "other"}}, "edges": [["Bolshoi_Ballet", "orgbased_in", "Moscow"], ["Kirov_Ballet", "orgbased_in", "Leningrad"], ["June_Anderson", "live_in", "U.S."], ["Carol_Vaness", "live_in", "U.S."]]}
    ...
    输出为{"nodes": {"Iowa": {"type": "loc"}, "Terry_Branstad": {"type": "peop"}, "National_Governors_'_Association": {"type": "org"}, "Booth_Gardner": {"type": "peop"}, "Washington": {"type": "loc"}, "Branstad": {"type": "peop"}}, "edges": [["Terry_Branstad", "live_in", "Iowa"], ["Terry_Branstad", "work_for", "National_Governors_'_Association"], ["Booth_Gardner", "work_for", "National_Governors_'_Association"], ["Booth_Gardner", "live_in", "Washington"], ["Branstad", "live_in", "Iowa"], ["Branstad", "work_for", "National_Governors_'_Association"]]}
    # 以下为测试样本，请输出json（字符串），不要输出多余的内容。确保输出的标签在给出的4种实体标签与5种关系标签之中！
'''


in_path = "data/CoNLL04/test_graph_serialized.jsonl"
out_path = "data/CoNLL04/naive_test_llm_answer.jsonl"
