...
for i, line in enumerate(trainlist):
    sample = {"tokens":list(), "entities":list(), "relations":list()}
    sample["tokens"] = list(line["text"])
    mention2entidx = dict()
    cnt = 0
    ...
    for ent in line['entities']:
        ...
        if ent[-2] not in ent_mention_contained:
            ...
            matched_span_list = match_sublist(sample["tokens"], list(ent[-2]))
            start, end = matched_span_list[0][0], matched_span_list[0][1]
            sample["entities"].append({"type":ent[-1], "start": start, "end": end})
            mention2entidx[ent[-2]] = cnt
            cnt += 1
        else:
           # print(f"样本{i+1}:实体：{ent[-2]}重复")
           pass
    ...
    for rel in line["relations"]:
        ...
        if (rel[0][-1], rel[1], rel[-1][-1]) not in rel_mention_contained:
            ...
            sample["relations"].append({"type": rel[1], "head": mention2entidx[rel[0][-1]], "tail":mention2entidx[rel[-1][-1]]})
        else:
            #print(f"样本{i+1}:关系：({rel[0][-1]}, {rel[1]}, {rel[-1][-1]})重复")
            pass
...
for j, line in enumerate(devlist):
    ...
    for i, rel in enumerate(line['relations']):
        ...
        if head[-1] not in ent_mention_contained:   # 若头实体提及没有出现过
            ...
            match_list_head = match_sublist(sample["tokens"], list(head[-1]))   # 将头实体边界映射到first
            assert len(match_list_head) > 0
            #head = [match_list_head[0][0], match_list_head[0][1], head[-1]]     # 修改头实体span
            sample["entities"].append({"type":"药物", "start": match_list_head[0][0], "end": match_list_head[0][1]})
        ...
        if tail[-1] not in ent_mention_contained:   # 若头实体提及没有出现过
            ...
            match_list_tail = match_sublist(sample["tokens"], list(tail[-1]))   # 将尾实体边界映射到first
            assert len(match_list_tail) > 0
            #head = [match_list_head[0][0], match_list_head[0][1], head[-1]]     # 修改头实体span
            sample["entities"].append({"type":"药物", "start": match_list_tail[0][0], "end": match_list_tail[0][1]})
...
write_json(sample_list, "text2dt.json")