...
（6）PART-WHOLE : 部分-整体关系。表示一个实体是另一个实体的组成部分。例如(Israel, Middle East)，即以色列是中东的一部分。也常用于地点之间的包含关系，如城市属于国家。
  ...
  # 3.输出：实体和关系抽取结果，采用json格式，包含nodes字段和edges字段，分别对应实体和关系抽取结果。nodes字段是一个字典，键为实体提及，值为一个字典，有一个键type、其值为实体类型。edges字段是一个列表，每一个元素是一个三元组（列表），分别对应头实体提及、关系类型和尾实体提及。
    ...
    输出为{"nodes": {"officials": {"type": "PER"}, "republican_guard": {"type": "ORG"}, "units": {"type": "PER"}, "artillery": {"type": "WEA"}, "shells": {"type": "WEA"}, "agents": {"type": "WEA"}}, "edges": [["units", "ORG-AFF", "republican_guard"], ["units", "ART", "shells"]]}
    （2）示例2：输入为"a source tell us enron is considering suing its own investment bankers for giving it bad financial advice .",
    输出为{"nodes": {"source": {"type": "PER"}, "us": {"type": "ORG"}, "enron": {"type": "ORG"}, "its": {"type": "ORG"}, "own": {"type": "ORG"}, "bankers": {"type": "ORG"}, "it": {"type": "ORG"}}, "edges": [["bankers", "ORG-AFF", "its"]]}
    # 以下为测试样本，请输出json（字符串），不要输出多余的内容。确保输出的标签在给出的7种实体标签与6种关系标签之中！
'''


in_path = "data/ACE2005/test_graph_serialized.jsonl"
out_path = "data/ACE2005/naive_test_llm_answer.jsonl"
